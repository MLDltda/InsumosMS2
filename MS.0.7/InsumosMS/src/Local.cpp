...
bool Local::checkExistencia(int num){
    if(num >= insumo.size()){
...
void Local::salvando(){
    ...
    if(fp.is_open()){
            for(int i = 0 ; i < insumo.size(); i++){
                ...
                fp << insumo[i]->getNome() << std::endl;
                fp << insumo[i]->getQuant() << std::endl;
                fp << insumo[i]->getValorUnitario() << std::endl;
                fp << insumo[i]->getDtVencimento() << std::endl;
                fp << insumo[i]->getNomeFabricante() << std::endl;
                fp << insumo[i]->getTipoInsumo() << std::endl;
                if(insumo[i]->getTipoInsumo() == 1){
                fp << insumo[i]->getTipo() << std::endl;
                fp << insumo[i]->getQuantDoses() << std::endl;
                fp << insumo[i]->getIntervalo() << std::endl;
                }
                if(insumo[i]->getTipoInsumo() == 2){
                fp << insumo[i]->getDosagem() << std::endl;
                fp << insumo[i]->getAdministracao() << std::endl;
                fp << insumo[i]->getDisponibilizacao() << std::endl;
                }
                if(insumo[i]->getTipoInsumo() == 3){
                fp << insumo[i]->getTipo() << std::endl;
                fp << insumo[i]->getDescricao() << std::endl;
                }
            }
            fp.close();
    }
