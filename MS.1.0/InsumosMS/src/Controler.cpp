...
Controler::Controler()
{
    ...
    locais[5].setNomeLocal("Amazonas");
...
void Controler::distribuiInsumo(int escolha){
    ...
    place = locais[escolha].checkDist(vet);
    ...
    if(place < 0){
        ...
        std::cout<<"Insumo ja Distribuido, somando a quantidade enviada"<<std::endl;
        locais[escolha].somaDist(place, quant);
        locais[0].novaQuant(ins, total);
        std::cout << "Insumo enviado com sucesso." << std::endl;
        std::cout << "Aperte enter para continuar." << std::endl;
        getchar();
...
void Controler::carregarDados(){
    std::vector<Insumo*> vet;
    Persistencia pers;
    for(int i = 0; i < 28; i++){
        vet = pers.lerInsumos(locais[i].getNomeLocal());
        locais[i].PreencheInsumo(vet);
    }
